...
EAI.Map = function(){
	 ...
	 * 链表头部节点
	 */
	var headNode = new EAI.MapNode({key : '', data : id+'_链表头部', next : null, previous : null});
	/**
	 * 链表指针
	 */
	var p = new EAI.MapNode({data : id+'_指针', next : headNode, previous : headNode});
	/**
	 * @description
	 ...
	 * 		String key -节点的Key
	 * @return
	 * 		EAI.MapNode node -指定key的节点
	 */ 
	var findByKey = function(key){
		var tp = new EAI.MapNode({next : headNode.next, previous : headNode});
		while(tp.next){
			if(tp.next.key == key){
				return tp.next;
			}else {
	...
	this.set = function(key,data){
		...
		if(node){
			...
			add(new EAI.MapNode({key : key, data : data}));
		}
		return true;
	}
	/**
