...
Ext.extend(Ext.ux.UploadDialog.Dialog, Ext.Window, {
	...
	createGrid : function() {
		...
		var cm = new Ext.grid.ColumnModel([{
					...
					renderer : function(v) {
					return "<a href=javascript:void(0) id='uploadDialog_grid_downLoad_id1' url="+ME.downUrl+" onclick='onClickFn()' style=color:blue;text-decoration:underline>"+v+"</a>";
				}
				}]);
...
//						.child(tb.x_buttons.upload.buttonSelector).dom[tb.x_buttons.upload.tooltipType] = this.i18n.upload_btn_start_tip;
	...
	removeFiles : function(file_records) {
		var ME=this;
		alert(Ext.encode(file_records));
		ME.grid_panel.getStore().remove(file_records);
		var url=ME.delUrl;
		var hidename=Ext.getCmp(ME.hiddenFieldId).text;
		var arrname=[];
			...
			if (btn == 'yes') {
				 Ext.Ajax.request({
						...
						success : function(result, request) {
							...
							if (returnStr) {
								Ext.each(file_records, function(record) {
									ME.grid_panel.getStore().remove(record);
							    }.createDelegate(this));
							}
							Ext.getCmp(ME.hiddenFieldId).text='';
						},
		...
		Ext.Ajax.request({//here
					...
					success : function(response, options) {
						...
						try {
							...
							if (hiddelid!=""&&json_response) {
								var v = Ext.getCmp(hiddelid).text;
								var arrstr=json_response.path.split("*");
								var str="";
								...
								if (v!=""&&v!=undefined){
									...
									//Ext.getCmp(this.hiddenFieldId).setText(str);
								}else{
