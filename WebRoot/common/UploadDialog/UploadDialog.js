...
var onClickFn =function(e){  
	    //var href = e.getTarget('a', 4, true);   
	     ...
	     var index_n=id.indexOf('_');
	     //var customerId=id.substring(0,index_n);//具体文件
	     var typeId=id.substring(0,index_n);//具体文件
	    var fname=document.getElementById(id).innerHTML;
    	/*var index=fname.lastIndexOf("\\");
	    var filename=fname.substring(index+1,fname.length);//文件名*/	  
	    if(element){   
	    	
	           var type =element.getAttribute('type');//类型文件
	           //var url='/EAI/upload?action=downLoad&parentId='+customerId+"&type="+type+"&filename="+encodeURI(fname)+"&num="+Math.random();
	           
	           var url="qmManage.do?action=downfiel&typeId="+typeId+"&type="+type+"&filename="+encodeURI(fname)+"&num="+Math.random();
	           if (!Ext.fly('frmDummy')) {
		       ...
		       //if (!Ext.fly('frmDummy')){       
		         ...
		         frm.action = url;   
		         frm.method = 'post';   
		         frm.target = '_blank'; //_self,_parent,_blank  
		         document.body.appendChild(frm);       
		        //}
		         alert(frm.action+"==");
...
Ext.extend(Ext.ux.UploadDialog.Dialog, Ext.Window, {
	...
	createGrid : function() {
		...
		var cm = new Ext.grid.ColumnModel([{
					...
					renderer : function(v) {
						//onClickFn()
					return "<a href=javascript:void(0) id="+ME.base_params.parentId+"_"+v+" onclick='onClickFn()' type="+ME.base_params.type+" style=color:blue;text-decoration:underline>"+v+"</a>";
				}
				}]);
...
//						.child(tb.x_buttons.upload.buttonSelector).dom[tb.x_buttons.upload.tooltipType] = this.i18n.upload_btn_start_tip;
		...
		Ext.MessageBox.confirm('确认删除', '确认要删除所选附件吗?', function(btn) {
					if (btn == 'yes') {
						...
						for (var i = 0, len = file_records.length; i < len; i++) {
							...
							var url=r.get('fileName');
							var name=url.lastIndexOf("\\");
							var picname=url.substring(name+1,url.length);
							var str="";
							for(var a=0;a<arrname.length;a++){
							   var index=arrname[a].indexOf(picname);
							   if(index==-1){
							     if(str==""){
							        str=arrname[a];
							     }else{
							      ...
							      deletepic=picname;
							      Ext.Ajax.request({
										url : "/EAI/upload?action=uploadDel",
										params : {
											//fileIds : delIds
											deletepic:deletepic,
											parentId:p.base_params.parentId,
											type:p.base_params.type
										},
										form : this.form,
					                    isUpload : true,
					                    scope : this,
										...
										success : function(result, request) {
											var rs=Ext.decode(result.responseText);
											//window.parent.msgWindow(rs.message);
							...
							Ext.getCmp(p.hiddenFieldId).setText(str);
							store.remove(r);

						}
						window.parent.msgWindow("文件删除成功！");
						/*// 去除最后一个分隔符
						if (delIds.length > 0) {
						...
						if (!Ext.isEmpty(delIds)) {
							Ext.Ajax.request({
										url : "/EAI/upload?action=uploadDel&deletepic="+deletepic+"&parentId="+p.base_params.parentId,
										,
										params : {
											fileIds : delIds
										}
	...
	uploadFile : function(record) {
		// this.form.submit();
		/*var parentId_lab=this.custFieldId;
		var parentId=Ext.getCmp(custid_lab).getValue();*/
		/*if(this.base_params.parentId==""||this.base_params.parentId==undefined){
		     self.parent.msgWindow("客户不可为空！");
		     return;
		}*/

		//alert(Ext.encode(this.base_params || this.baseParams || this.params));
		//return;
		Ext.Ajax.request({
					url : this.url,
	...
	onAjaxSuccess : function(response, options) {
		...
		try {
			...
			var json_response = Ext.decode(text);
			if (hiddelid!="") {
				var v = Ext.getCmp(hiddelid).text;
				var arrstr=json_response.pictureArr.split("*");
				var str="";
				...
				if (v!=""&&v!=undefined){
					Ext.getCmp(hiddelid).setText(v + ","+ str);
				}else{
					Ext.getCmp(hiddelid).setText(str);
					
				}
			}
			//self.parent.msgWindow(json_response.message);
		} catch (e) {
			//self.parent.msgWindow("文件上传失败！");
		}
		var data = {
